...
class HTMLReportGenerator:
    ...
    def generate(stats):
        ...
        <head>
            ...
            <style>
                * {{ box-sizing: border-box; }}
                ...
                .table-wrapper {{ max-height: 600px; overflow-y: auto; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.04); }}
                table {{ width: 100%; border-collapse: collapse; background: white; }}
                th {{ background: #f1f5f9; color: #1e293b; font-weight: 600; padding: 12px 16px; text-align: left; position: sticky; top: 0; z-index: 2; }}
                ...
                .links-grid {{ display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 20px; }}
                .links-column {{ background: white; border-radius: 8px; padding: 10px; border: 1px solid #e2e8f0; overflow-y: auto; max-height: 300px; }}
        ...
        for idx, (url, s) in enumerate(stats.items()):
            ...
            abs_links = s.get('absolute_links', [])
            rel_links = s.get('relative_links', [])
            rem_links = s.get('removed_links', [])
                            ...
                            <td colspan="6">
                                <div class="detail-content">
                                    <div class="links-grid">
                                        <div class="links-column" style="border-left: 3px solid #3b82f6;">
                                            <h4 style="color: #1e40af;">🌐 Абсолютные ({len(abs_links)})</h4>
                                            <ul>
            """
            if abs_links:
            ...
            html += """
                                        ...
                                        <div class="links-column" style="border-left: 3px solid #f59e0b;">
                                            <h4 style="color: #92400e;">🔗 Относительные ({count_rel})</h4>
                                            <ul>
            """.replace("{count_rel}", str(len(rel_links)))
            if rel_links:
            ...
            html += """
                                        ...
                                        <div class="links-column" style="border-left: 3px solid #ef4444;">
                                            <h4 style="color: #991b1b;">❌ Невалидные ({count_rem})</h4>
                                            <ul>
            """.replace("{count_rem}", str(len(rem_links)))
            if rem_links:
